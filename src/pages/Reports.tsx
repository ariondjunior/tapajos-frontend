      ...
      <div className="card">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center">
            ...
            <h3 className="text-lg font-semibold">Extrato total</h3>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {extratoDiario.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-4 py-6 text-center text-sm text-gray-500">Nenhuma movimentação</td>
                  </tr>
                ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
          ...
          <div className="card p-4">
            ...
            <p className="text-2xl font-semibold text-red-600">
              {formatCurrency(
                transactions.filter(t => t.type === 'debit').reduce((s, t) => s + (t.amount || 0), 0)
              )}
            </p>
          </div>
          <div className="card p-4">
            <p className="text-sm font-medium text-secondary-600">Total Transações</p>
            <p className="text-2xl font-semibold text-secondary-900">{transactions.length}</p>
          </div>
        </div>
